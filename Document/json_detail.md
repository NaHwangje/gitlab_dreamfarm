...
- `"UTIMER"`:
    - 작동기 타이머 설정 (1분단위)
    - 1바이트 단위, 16진수 표기법, size : hexString 360
    - 1일 = 24시간 = 1440분 = 180개 x2 (x2하는 이유는 1개의 분마다 켜진 상태와 꺼진 상태 2가지가 있기 때문)
    - 1비트는 0,1로 표현이 가능한데, 0이면 꺼진 상태 1이면 켜진 상태로 생각하면 된다. 
    - 16진수 2개는 0000 0000 이니 0000 0001 이면 0분부터 1분까지 on이라는 의미임
      <br>
